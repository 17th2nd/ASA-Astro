...
`astro store build --store var/astro-store3 --frontier --as-of 2026-09-04T08:00:00Z --evidence-fragment var/tess-fragment.json --universe-out var/universe-real-frontier-tess-v2.json` — **done.** Universe UNI-1522e9e4…, 95,659 entities · 187,400 evidence · 135,517 relationships; kernel seq 2,470,595, digest sha256:1a6fce51…; parse+merge 410 s, kernel load 4,634 s (the `_supports_count` scan, unchanged). Frontier as the build derived it: gaps 48,011 (same as store2); geometry `near` 9,660 · `comparison_star_for` 1,952 · **`member_of` 41 endorsed · 20 unevaluated · 183 rejected on proper motion** — the rebuild reproduces the offline re-test exactly; claims 69,479 · contradictions 439 · derived distances 4,391; tiles 648, 165 gap tiles. `tools/candidate_findings.py` on the v2 universe: 41 kinematic members (all evidenced), 20 untrusted (asserted without evidence), 0 rejected (no longer asserted), drift 419 hosts / 546 planets, 6 undetermined (`var/runs/candidate-findings-store3.json`). The 50 pc set re-cut from v2 (`var/universe-50pc.json`, same 613/959/4 census) and the navigator rebuilt on it (universe UNI-598d5f24…) and republished to the same link. `astro frontier` against store3 (`var/runs/frontier-report-store3.json`, kernel seq 2,470,595): 135,956 registered relationships; blank spaces 48,011; semantic edges **2,317** (comparison_star_for 1,952 · hosts 345 · **member_of 20** — the unevaluated memberships now sit on record as semantic edges, +20 on store2); derived and endorsed near 9,660 · member_of 41; disputes 439 over 432 entities; 165 gap tiles. ⚠ store2 is superseded; delete when disk is needed.
